...
class QdrantDB:
    def __init__(self, collection_name="image_collection", host="localhost", port=6333, in_memory=False, prefer_grpc=False, api_key=None, timeout=10.0):
        ...
        self.vector_size = 512 # Define default vector size, can be made configurable
        ...
        if in_memory:
            try:
                ...
                print(f"Failed to initialize QdrantDB in-memory: {e}")
                # For in-memory, failure is critical, so re-raise or handle as per app policy
                raise ConnectionError(f"Qdrant in-memory initialization failed: {e}") from e
        else:
            try:
                self.client = QdrantClient(host=host, port=port, prefer_grpc=prefer_grpc, api_key=api_key, timeout=timeout)
                print(f"Attempting to connect to QdrantDB at {host}:{port}...")
                # A simple check to confirm connectivity (e.g., get cluster info)
                # This will raise an exception if the server is not reachable within the timeout
                self.client.get_collections() # Or a lighter-weight ping if available
                print(f"Successfully connected to QdrantDB at {host}:{port}.")
            except Exception as e:
                print(f"CRITICAL: Failed to connect to QdrantDB at {host}:{port}. Error: {e}")
                print("Please ensure Qdrant server is running and accessible.")
                # Option 1: Raise an error to be caught by calling code
                raise ConnectionError(f"Failed to connect to Qdrant at {host}:{port}. Is it running?") from e
                # Option 2: Keep self.client as None, and let other methods handle it (less ideal for init)

        if self.client: # Proceed only if client was successfully initialized
            try:
                self.client.get_collection(collection_name=self.collection_name)
                print(f"Collection '{self.collection_name}' already exists.")
            except Exception as e:
                # Check if it's a "not found" type error vs. a connection error
                # This part of the logic depends on how qdrant_client signals "not found"
                # For now, we assume any error here while client is valid means collection might not exist
                print(f"Collection '{self.collection_name}' not found or error accessing: {e}. Attempting to create.")
                try:
                    self.client.recreate_collection(
                        collection_name=self.collection_name,
                        vectors_config=models.VectorParams(
                            size=self.vector_size, # Ensure this is defined
                    ...
                    # If collection creation fails, this is also a critical issue.
                    # Depending on app policy, you might want to raise an error here too.
                    raise ConnectionError(f"Qdrant connected, but failed to create collection '{self.collection_name}': {create_e}") from create_e
        else:
            # This case should ideally be handled by the ConnectionError raised during client init
            print("Qdrant client not initialized. Database operations will fail.")
